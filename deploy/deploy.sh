...
compose_candidate pull postgres migrate stocks-bot publications-bot
...
if ! compose_candidate up \
  ...
  --wait \
  --wait-timeout 180 \
  stocks-bot publications-bot; then
  echo "Release failed its container health checks." >&2

  if [[ -f "$RELEASE_FILE" ]]; then
    echo "Restoring the previous healthy application image..."
    compose_release pull stocks-bot publications-bot
    compose_release up \
      -d \
      --remove-orphans \
      --wait \
      --wait-timeout 180 \
      stocks-bot publications-bot
  fi
