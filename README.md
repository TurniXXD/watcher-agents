...
- **Stocks Watcher** monitors SEC filings, auto-discovered issuer feeds, GDELT news, TradingView symbol news, curated first-party company intelligence for selected AI-infrastructure stocks, FINVIZ insider transactions, Zacks rank/quote snapshots, Earnings Whispers earnings snapshots, and price snapshots.
...
4. Download the accepted Piper voice models, then build, migrate, and start all bots:
   ...
   docker compose ps
   docker compose logs -f stocks-bot publications-bot news-bot mu-clubs-monitor brno-events-agent briefing-bot maintenance-agent
   ```
...
Phase 5 adds targeted event analysis, primary-driver/redundancy classification, weighted signal groups, source-aware data coverage, change detection, and persistent versioned thesis state. Ollama prompts use a bounded source excerpt and compact prior-event context to fit the configured context window; the complete source evidence remains persisted. Required causal explanation, risks, and confidence must still validate before any thesis is stored. Full Ollama analysis is skipped when targeted analysis finds no meaningful thesis change. `/thesis SYMBOL` shows the latest thesis, confidence, attention, bull/bear/net scores, catalysts, risks, and material data gaps.
