...
export const renderReport = (title: string, findings: Finding[]): string => {
  const important = findings.filter((finding) => ['CRITICAL', 'HIGH', 'MEDIUM'].includes(finding.severity)).slice(0, 15);
  if (important.length === 0) return `🛠 ${title}\n\nNo important maintenance findings.`;
  return [
    `🛠 ${title}`,
    '',
...
export const createMaintenanceBot = (
  ...
  const summary = async () => {
    ...
    const lines = findings.slice(0, 15).map(
      (finding) => `${severityEmoji[finding.severity]} ${finding.agentName}: ${finding.title}`,
    );
    return ['🛠 Maintenance summary', '', ...(lines.length ? lines : ['No open findings.'])].join('\n');
  };
  bot.command('start', async (context) => context.reply(
  ...
  bot.command('help', async (context) => context.reply(
  ...
  bot.command('run', async (context) => {
    ...
    await context.reply(renderReport('Manual maintenance report', result.findings));
  });
  bot.command('recommendations', async (context) => {
    const recommendations = await store.listRecommendations({ status: 'PROPOSED', limit: 15 });
    await context.reply([
      '🧰 Proposed maintenance changes',
      '',
      ...(recommendations.length
        ? recommendations.map((item) => `• ${item.agentName}: ${item.title} (${Math.round(item.confidence * 100)}%)`)
        : ['No proposals.']),
      '',
  ...
  bot.command('updates', async (context) => {
    try {
      const entries = changelogEntries(await readFile(changelogPath, 'utf8')).slice(-3).reverse();
      await context.reply([
      ...
      logger.warn({ err: error, changelogPath }, 'Maintenance changelog command failed');
      await context.reply('Project changelog is unavailable.');
    }
  });
  bot.catch((error) => logger.error({ err: error.error }, 'Maintenance Telegram update failed'));
  return { bot, summary };
};
...
export const announceChangelog = async (
  ...
  try {
    ...
    logger.warn({ err: error, path }, 'Maintenance changelog could not be read');
    return;
  }
  for (const entry of changelogEntries(markdown)) {
    for (const chatId of allowedIds) {
      if (!(await store.claimChangeAnnouncement(entry.hash, BigInt(chatId), entry.title))) continue;
      await bot.api.sendMessage(chatId, `🆕 Project update: ${entry.title}\n\n${entry.body}`);
    }
  }
};
