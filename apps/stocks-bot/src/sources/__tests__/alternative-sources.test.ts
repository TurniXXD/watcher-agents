...
describe('TradingViewNewsSource', () => {
  it('normalizes symbol headlines and enriches them with article text when available', async () => {
    const mockFetch = vi.fn(async (input: RequestInfo | URL) => {
      const url = requestUrl(input);
      if (url.includes('/v2/view/headlines/symbol')) {
        const request = new URL(url);
        expect(request.searchParams.get('symbol')).toBe('NASDAQ:CRDO');
        expect(request.searchParams.get('client')).toBe('web');
        return Response.json({
          items: [
            {
              id: 'zacks:0b87f7ef3094b:0',
              title:
                "Will Credo's Neocloud Expansion Broaden Its AEC Growth Runway?",
              published: 1788500000,
              storyPath:
                '/news/zacks:0b87f7ef3094b:0-will-credo-s-neocloud-expansion-broaden-its-aec-growth-runway/',
              permission: 'headline',
              urgency: 2,
              provider: { id: 'zacks', name: 'Zacks' },
              relatedSymbols: [{ symbol: 'NASDAQ:CRDO' }],
            },
          ],
        });
      }
      if (url.includes('/news/zacks:0b87f7ef3094b')) {
        return new Response(`
          <html><head>
            <script type="application/ld+json">
              {
                ...
                "articleBody": "Credo continues to see a healthy growth runway for its Active Electrical Cables business."
              }
            </script>
          </head><body></body></html>
        `);
    ...
    const source = new TradingViewNewsSource(mockFetch);
    ...
    expect(item).toMatchObject({
      ...
      externalId: 'zacks:0b87f7ef3094b:0',
      title:
        "CRDO TradingView: Will Credo's Neocloud Expansion Broaden Its AEC Growth Runway?",
      url: 'https://www.tradingview.com/news/zacks:0b87f7ef3094b:0-will-credo-s-neocloud-expansion-broaden-its-aec-growth-runway/',
      sourceType: 'NEWS',
      category: 'NEWS',
      normalizedFacts: {
        provider: 'Zacks Investment Research',
        tradingViewSymbol: 'NASDAQ:CRDO',
        relatedSymbols: ['NASDAQ:CRDO'],
        permission: 'headline',
        urgency: 2,
      },
      metadata: {
    ...
    expect(item?.publishedAt?.toISOString()).toBe('2026-09-04T10:00:00.000Z');
    expect(item?.content).toContain('Article text: Credo continues');
  });
});
