...
/feed_add PROFILE URL [NAME] — add an RSS/Atom feed
...
News Watcher is one private, self-hosted Telegram bot with two editorial profiles: *Czech* for news centered on Czechia and *Global* for important international developments. Both profiles share one reliable ingestion and analysis engine while keeping their feeds and topics separate.

*What it does*
• Reads operator-configured RSS and Atom feeds and normalizes their articles into one format.
...
*Key advantages*
• One engine, two profiles: less duplicated code and consistent ranking, while Czech and Global sources remain independently configurable.
...
*How to use it*
1. Add feeds with \`/feed_add czech URL NAME\` or \`/feed_add global URL NAME\`.
2. Add editorial interests with \`/topic_add czech TOPIC\` and \`/topic_add global TOPIC\`. With no topics, the bot ranks general significance for that profile.
3. Review configuration with \`/feeds\` and \`/topics\`.
